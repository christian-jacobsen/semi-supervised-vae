...
class ssvae(nn.Module):
    def __init__(self, data_channels, initial_features, denseblocks, growth_rate, n_latent, prior = 'std_norm', activations = nn.GELU()):
        ...
        self.zl_logvar = torch.Tensor([np.log(0.005), np.log(0.005)])
            
        self.prior_logvar = self.prior_logvar.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        self.zl_logvar = self.zl_logvar.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    def compute_loss(self, xu, xl, zl):
        # in this case, we have xu = unlabeled data, xl = labeled data, zl = representation of labeled data
        #freebits = 0
        zmu, zlogvar, z, xmu, xlogvar = self.forward(torch.cat((xu, xl), 0))
        l_rec = -torch.sum(self.gaussian_log_prob(torch.cat((xu,xl),0), xmu, xlogvar), 1)
